...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['producto_id']) && !isset($_POST['update_cuotas']) && !isset($_POST['assign_producto'])) {
        ...
        $sql = "INSERT INTO usuarios (username, role, vendedor_id) VALUES ('$subvendedor_username', 'subvendedor', $vendedor_id)";
        if ($conn->query($sql) === TRUE) {
...
<body>
    <div class="container mt-5">
        ...
        <table class="table">
            ...
            <tbody>
                <?php while ($row = $ventas_consolidadas->fetch_assoc()): ?>
                <tr>
                    <td><?= $row['username'] ?></td>
                    <td><?= formatMoney($row['total_recaudado']) ?></td>
                </tr>
                <?php endwhile; ?>
            </tbody>
        </table>
