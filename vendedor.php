...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['producto_id']) && !isset($_POST['update_cuotas']) && !isset($_POST['assign_producto']) && !isset($_POST['update_subvendedor']) && !isset($_POST['delete_subvendedor'])) {
        ...
        $sql = "INSERT INTO usuarios (username, role, vendedor_id) VALUES ('$subvendedor_username', 'vendedor', $vendedor_id)";
        if ($conn->query($sql) === TRUE) {
        ...
        $subvendedores = $conn->query("SELECT * FROM usuarios WHERE vendedor_id = $vendedor_id");
    } elseif (isset($_POST['update_subvendedor'])) {
        $subvendedor_id = $_POST['subvendedor_id'];
        $subvendedor_username = $_POST['subvendedor_username'];
        $sql = "UPDATE usuarios SET username = '$subvendedor_username' WHERE id = $subvendedor_id AND vendedor_id = $vendedor_id";
        if ($conn->query($sql) === TRUE) {
            echo "<script>alert('Subvendedor actualizado correctamente');</script>";
        } else {
            echo "<script>alert('Error al actualizar subvendedor: " . $conn->error . "');</script>";
        }
        // Recargar la lista de subvendedores
        $subvendedores = $conn->query("SELECT * FROM usuarios WHERE vendedor_id = $vendedor_id");
    } elseif (isset($_POST['delete_subvendedor'])) {
        $subvendedor_id = $_POST['subvendedor_id'];
        $sql = "DELETE FROM usuarios WHERE id = $subvendedor_id AND vendedor_id = $vendedor_id";
        if ($conn->query($sql) === TRUE) {
            echo "<script>alert('Subvendedor eliminado correctamente');</script>";
        } else {
            echo "<script>alert('Error al eliminar subvendedor: " . $conn->error . "');</script>";
        }
        // Recargar la lista de subvendedores
        $subvendedores = $conn->query("SELECT * FROM usuarios WHERE vendedor_id = $vendedor_id");
        ...
        foreach ($_POST['subvendedor_id'] as $producto_id => $subvendedor_id) {
            ...
            $sql = "UPDATE productos SET subvendedor_id = " . ($subvendedor_id ? $subvendedor_id : "NULL") . ", total_pagado = $total_pagado WHERE id = $producto_id AND vendedor_id = $vendedor_id";
            if ($conn->query($sql) === TRUE) {
...
// Calcular el total recaudado por cada subvendedor
$ventas_consolidadas = $conn->query("SELECT u.username, SUM(p.total_pagado) as total_recaudado FROM productos p JOIN usuarios u ON p.subvendedor_id = u.id WHERE p.vendedor_id = $vendedor_id GROUP BY u.id");
...
<body>
    <div class="container mt-5">
            ...
            <?php while ($subvendedor = $subvendedores->fetch_assoc()): ?>
                <li class="list-group-item">
                    <form method="POST" class="form-inline">
                        <input type="hidden" name="subvendedor_id" value="<?= $subvendedor['id'] ?>">
                        <input type="text" class="form-control mb-2 mr-sm-2" name="subvendedor_username" value="<?= $subvendedor['username'] ?>" required>
                        <button type="submit" name="update_subvendedor" class="btn btn-warning mb-2 mr-sm-2">Actualizar</button>
                        <button type="submit" name="delete_subvendedor" class="btn btn-danger mb-2" onclick="return confirm('¿Está seguro de que desea eliminar este subvendedor?');">Eliminar</button>
                    </form>
                </li>
            <?php endwhile; ?>
        </ul>
        ...
        <h3 class="mt-4">Ventas Consolidadas por Subvendedores</h3>
        <table class="table">
            <thead>
                <tr>
                    <th>Subvendedores</th>
            ...
            <tbody>
                <?php while ($venta = $ventas_consolidadas->fetch_assoc()): ?>
                    <tr>
                        <td><?= $venta['username'] ?></td>
                        <td><?= formatMoney($venta['total_recaudado']) ?></td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
        </table>
